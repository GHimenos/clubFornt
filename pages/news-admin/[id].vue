        ...
        <div v-for="item in news">
          ...
          >
            ...
            <div class="col-span-3">
              ...
              />
              <div class="mb-2 colorGray text-sm mt-4">Текст новости</div>
...
onMounted(async () => {
  ...
  const responseInfo = await client<Newsadmin>(
    "/newsadmins/?populate=cover&pagination[pageSize]=1000000",
    {
      method: "GET",
    }
  );
  news.value = responseInfo.data;
...
const onSubmit = async (item) => {
  ...
  timer();
  try {
    await update<Newsadmin>("newsadmins", item.id, {
...
const onPublic = async (item) => {
  ...
  try {
    ...
    await create<Newspublic>("newspublics", {
...
const offPublic = async (item) => {
  ...
  timer();
  try {
    const responsePublic = await find<Newspublic>(
    ...
    await _delete<Newspublic>("newspublics", publicid.value[0].id);
    await update<Newsadmin>("newsadmins", item.id, {
      publish: false,
    });
    message.value = "Успешно";
...
async function addCover(item) {
  try {
    ...
    const { data } = await client<Newsadmin>(`/newsadmins/` + item.id, {
      method: "PUT",
      body: formData,
    });
  } catch (e) {}
