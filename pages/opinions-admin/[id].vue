...
onMounted(async () => {
  ...
  const responseInfo = await client<Opinionadmin>(
    "/opinionadmins/?populate=cover&pagination[pageSize]=1000000",
    {
      method: "GET",
    }
  );
  opinions.value = responseInfo.data;
...
const offPublic = async (item) => {
  ...
  timer();
  try {
    const responsePublic = await find<Opinionpublic>(
    ...
    await _delete<Opinionpublic>("opinionpublics", publicid.value[0].id);
    await update<Opinionadmin>("opinionadmins", item.id, {
      publish: false,
    });
    message.value = "Успішно";
...
async function addCover(item) {
  try {
    ...
    const { data } = await client<Opinionadmin>(`/opinionadmins/` + item.id, {
      method: "PUT",
      body: formData,
    });
  } catch (e) {
